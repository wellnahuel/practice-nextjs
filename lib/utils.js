...
export async function getPathFromId() {
    ...
    const ids = items.map(item => {
        return {
            params: {
                id: item.id.toString() //esto lo hago porque me tiraba error , porque pedia que sea string el id
            }
        }
    });
    return ids;
...
export async function getItemData(id) {
    const items = await getItems();
    const product = items.find(item => item.id.toString() == id) //igual que por lo de arriba
